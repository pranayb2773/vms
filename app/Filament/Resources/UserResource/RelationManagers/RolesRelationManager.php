                            ...
                            ->suffixAction(
                                Forms\Components\Actions\Action::make('all')
                                    ...
                                    ->action(function (Set $set, $state) {
                                        $ids = Role::whereNotIn('id', $state)?->pluck('id')?->map(fn ($id): string => (string) $id)?->toArray();
                                        $set('recordId', array_merge($ids, $state));
                                    })
                            )
                    ])
                    ->recordTitle(fn (Role $record): string => $record->name . ' - ' . str($record->guard_name)->ucfirst()->value())
